...
if __name__ == '__main__':
    ...
    sf.write(os.path.join(outdir, 'out_wah_fuzz.wav'), wah_fuzz_output, samplerate = fs)
    sf.write(os.path.join(outdir, 'out_delay.wav'), delay_output, samplerate = fs)
    sf.write(os.path.join(outdir, 'out_overdrive.wav'), overdrive_output, samplerate = fs)
    sf.write(os.path.join(outdir, 'pan_output.wav'), pan_output, samplerate = fs)
    sf.write(os.path.join(outdir, 'schroeder1_output.wav'), schroeder1_output, samplerate = fs)
    sf.write(os.path.join(outdir, 'output_files/moorer_output.wav'), moorer_output, samplerate = fs)
